...
route.post("/messages", function(request, response) {
    ...
    request.on('end', function() {
        ...
        messageData.content = messageData.content.split("<").join("&lt;")
        messageData.content = messageData.content.split(">").join("&gt;")
        if (session[messageData.accessToken] || session[messageData.accessToken] == messageData.user) {
            mongo.insert('messages', messageData)
...
route.socket(function(data, connection) {
    console.log(data)
    connection.sendUTF("ACKNOWLEDGEMENT SUCCESSFUL")
})
